...
test_that("get_hydro_line works on a small area", {
  skip_on_cran()
  skip_on_ci()
  ...
  expect_true(all(c("TYPE", "NATURE", "NAME", "OFFSET") %in% names(hydro_line)))
  expect_true(sf::st_crs(hydro_line)$epsg == 2154)
  expect_true(all(hydro_line$TYPE %in% c("RUI", "RIN")))
})
