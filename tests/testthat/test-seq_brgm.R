fake_brgm_zip <- function(dep, cache){

  fake_sf <- sf::st_sf(
    data.frame(ID = 1),
    geometry = sf::st_sfc(sf::st_point(c(0, 0)), crs = 2154)
  )

  # Write shapefile to temporary directory
  sf_path <- tempfile()
  on.exit(unlink(sf_path, recursive = T))

  dir.create(sf_path)
  sf::write_sf(fake_sf, file.path(sf_path, "S_FGEOL.shp"), quiet = TRUE)
  base::writeLines("", file.path(sf_path, "S_FGEOL.qml"))

  # Create ZIP (base R)
  zip_name <- sprintf("GEO050K_HARM_%s.zip", pad_left(dep, 3))
  zip_path <- file.path(cache, zip_name)
  utils::zip(
    zipfile = zip_path,
    files = list.files(sf_path, full.names = TRUE),
    flags = c("-j", "-q")
  )

  return(zip_path)
}

test_that("seq_brgm() works for one dep", {
  brgm_cache <- file.path(tempdir(), "brgm")
  dir.create(brgm_cache)
  seq_cache <- file.path(tempdir(), "seq")
  dir.create(seq_cache)
  on.exit(unlink(c(brgm_cache, seq_cache), recursive = TRUE))

  m <- fake_matrice(id = "TEST")
  m_path <- file.path(seq_cache, "TEST_matrice.xlsx")
  openxlsx2::write_xlsx(m, m_path)

  p <- fake_parca(dep = 29)
  p_path <- seq_write(p, "parca", dirname = seq_cache)

  dep29 <- fake_brgm_zip(29, cache = brgm_cache)
  brgm <- seq_brgm(dirname = seq_cache, cache = brgm_cache, verbose = FALSE)

  expect_s3_class(brgm, "sf")
  expect_shape(brgm, dim = c(1,2))
  expect_length(list.files(seq_cache, pattern = "\\.qml$"), 1)
})

test_that("seq_brgm() works for multiple dep", {
  brgm_cache <- file.path(tempdir(), "brgm")
  dir.create(brgm_cache)
  seq_cache <- file.path(tempdir(), "seq")
  dir.create(seq_cache)
  on.exit(unlink(c(brgm_cache, seq_cache), recursive = TRUE))

  m <- fake_matrice(id = "TEST")
  m_path <- file.path(seq_cache, "TEST_matrice.xlsx")
  openxlsx2::write_xlsx(m, m_path)

  p <- fake_parca(dep = 1:2)
  p_path <- seq_write(p, "parca", dirname = seq_cache)

  dep1 <- fake_brgm_zip(1, cache = brgm_cache)
  dep2 <- fake_brgm_zip(2, cache = brgm_cache)
  brgm <- seq_brgm(dirname = seq_cache, cache = brgm_cache, verbose = FALSE)

  expect_s3_class(brgm, "sf")
  expect_shape(brgm, dim = c(2,2))
  expect_length(list.files(seq_cache, pattern = "\\.qml$"), 1)
})




