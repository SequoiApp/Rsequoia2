...
for the area covering \`x\` expanded with a buffer. The result is
returned as a masked \`SpatRaster\`, clipped to the input geometry to
...
- type:
  ...
  \`"rgb"\` - true-color orthophoto - \`"irc"\` - near-infrared
  orthophoto
