...
An \`sf\` object of type \`POLYGON\` containing infrastructure features
with standardized fields, including: \* \`TYPE\` - Infrastructure type
code: - \`AER\` = Aerodrome runway - \`BAT\` = Building - \`CIM\` =
Cemetery - \`CST\` = Surface construction - \`HAB\` = Residential area -
\`SPO\` = Sports ground - \`VIL\` = Urbanized area (importance 1-2) \*
\`NAME\` - Toponym when available \* \`SOURCE\` - Data source
(\`IGNF_BDTOPO_V3\`)
