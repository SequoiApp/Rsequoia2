...
point features from BDTOPO, normalizes them, and returns a combined
\`sf\` point layer.
...
An \`sf\` object containing hydrographic point features with four
fields: \* \`TYPE\` - hydrographic class - \`MAR\` = Pond \*
\`NATURE\` - Original BDTOPO nature field \* \`NAME\` - Official
hydrographic name (when available) \* \`ROTATION\` - Rotation or
orientation
