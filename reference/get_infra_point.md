...
Infrastructure type code derived from BDTOPO nature values: - \`PYL\` =
Pylon / antenna - \`CLO\` = Steeple - \`CRX\` = Cross or calvary -
\`EOL\` = Wind turbine - \`CST\` = Other point construction - \`GRO\` =
Cave - \`GOU\` = Sinkhole - \`ORO\` = Other orographic detail \*
