...
- ...:
  ...
  \`data.frame\` Each \`data.frame\` is wrote to a different sheet name.
  If \`...\` contain named arg, name is used as sheet name else variable
  name is used.
...
- data_table:
  ...
  \`logical\` If \`TRUE\`, data table is set up with total row. Default
  to \`FALSE\`.
