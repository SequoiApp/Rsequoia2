...
- rtol:
  ...
  Relative tolerance for surface correction within a UG (default 0.10).
...
- Identifying the dominant row per UG (largest total surface).

- Correcting rows that differ from the dominant description and whose
  surface is smaller than `atol` and represents less than `rtol` of UG
  total.

- Returning the corrected UA and rechecking inconsistencies.
