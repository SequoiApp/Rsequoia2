...
line features from BDTOPO, normalizes them, and returns a combined
\`sf\` linestring layer.
...
\* \`TYPE\` - hydrographic class - \`RUP\` = Permanent hydrographic
line - \`RUI\` = Intermittent hydrographic line \* \`NATURE\` - Original
BDTOPO nature field \* \`NAME\` - Official hydrographic name (when
available) \* \`OFFSET\` - Offset information
