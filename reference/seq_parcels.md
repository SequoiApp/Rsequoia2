...
Sequoia project, aggregates surfaces at the \`parcelle\` and
...
\[seq_write()\], respecting the \`dirname\`, \`overwrite\`, and
\`verbose\` parameters.

The function performs two levels of spatial aggregation ande create 4
layers:

1\. \*\*PF (Parcelle)\*\* Rows of the UA are grouped using the
\`parcelle\` field, and corrected surface areas are summed. A polygon
