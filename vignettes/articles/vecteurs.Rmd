...
Si l'utilisateur opte pour le procesus *Sequoia*, la fonction `seq_mnhm()` travaille directement avec le dossier normalisé en récupérant la couche *PARCA* comme zone d'étude. Elle télécharge l'ensemble des zonages intersectés avec `get_mnhn()` et les enregistre individuellement le dossier.
...
La fonction `get_ser_pdf()` permet de télécharger les fiches descriptives des sylvoécorégions.

Elle requiert un `sf` ou `data.frame` en argument `ser` ainsi qu'un `dirname` (le chemin du dossier) où télécharger les fiches `.pdf`.
...
La fonction `seq_patrimony()` récupère la couche *PARCA* du dossier normalisé et l'utilise comme zone d'étude. Elle télécharge l'ensemble des couches intersectés avec `get_patrimony()` et les enregistre individuellement le dossier.

# 5. SOL

## 5.1 Les bases géologiques départementales du BRGM
